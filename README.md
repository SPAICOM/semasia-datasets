...
[![HuggingFace](https://img.shields.io/badge/HuggingFace-SEMASIA-white?logo=huggingface&logoColor=FF9D00)](https://huggingface.co/collections/spaicom-lab/semasia)
...
<p align="center">
  <img src="img/conceptual_space.png" alt="Two-dimensional t-SNE projection of the latent space" width="80%">
  <br>
  <em>Two-dimensional t-SNE projection of the <code>aimv2_1b_patch14_224.apple_pt</code> 2048-dimensional latent space, populated with samples from seven SEMASIA benchmarks. Each benchmark forms its own cluster, but semantically overlapping concepts collapse onto shared neighborhoods regardless of source: flower images from Oxford Flowers and CIFAR-100 occupy the same region, as do large mammals and vehicles drawn from CIFAR-10, CIFAR-100, and Tiny-ImageNet (highlighted with the related origin dataset color).</em>
</p>
