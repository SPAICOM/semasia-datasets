        ...
        if isinstance(sample, dict) and 'digit' in sample:
            return np.array(
                [
                    int(
                        ''.join(
                            str(int(d.item()) if hasattr(d, 'item') else int(d))
                            for d in row[col]['digit']
                        )
                    )
                    for row in ds
                ]
...
def _(dataset_ui):
    ...
    mo.stop(
        not models_by_split,
        mo.callout(
            mo.md(f'Could not fetch repo info for **{_repo}**. Check your HF token.'),
            kind='danger',
        ),
    )
...
def _(models_by_split, split_ui):
    ...
    mo.stop(
        not _models,
        mo.callout(
            mo.md(f'No models found for split **{split_ui.value}**.'), kind='warn'
        ),
    )
...
def _(dataset_ui, model_ui, split_ui):
    ...
    with mo.status.spinner(title=f'Loading {_repo} ({split_ui.value} split)…'):
        ds = load_dataset(_repo, model_ui.value, split=split_ui.value).with_format(
            'torch'
        )
    mo.callout(
        mo.md(
            f'**{dataset_ui.value}** `{split_ui.value}` — '
            f'{len(ds):,} samples  |  embedding dim: '
            f'{ds[0]["embedding"].shape[-1]}'
        ),
        kind='success',
    )
    return (ds,)
...
def _(X2d, attr_ui, chart, dataset_ui, idx, orig_ds, trace_to_global, y):
    ...
    for _pt in _pts:
        ...
        _rows.append(
            {
                'dataset_idx': _ds_idx,
                'class': _class_names[_lbl]
                if 0 <= _lbl < len(_class_names)
                else str(_lbl),
                'label': _lbl,
...
def _(attr_ui, dataset_ui, idx, latent, n_dims_ui, pc_method_ui, y):
    ...
    with mo.status.spinner(
        title=f'Running {_method} ({_n} components) for parallel coords…'
    ):
        if _method == 'PCA':
...
def _(df_pc, pc_label_col):
    ...
    pc_widget = mo.ui.anywidget(
        _ParallelCoordinates(df_pc, height=500, color_by=pc_label_col)
    )
    pc_widget
    return (pc_widget,)
...
def _(dataset_ui, df_pc, orig_ds, pc_label_col, pc_widget, sel_ds_indices):
    ...
    _pc_col = df_pc[pc_label_col]
    _titles = []
    for j in range(len(_ds_indices)):
        _pc_idx = _pc_col[_sample[j]]
        _name = (
            _pc_class_names[_pc_idx] if _pc_idx < len(_pc_class_names) else str(_pc_idx)
        )
        _titles.append(f'{_name}\n#{_ds_indices[j]}')
    _fig = show_images(_pil_images, _titles)
